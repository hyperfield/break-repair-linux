#!/usr/bin/env bash

set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "$SCRIPT_DIR/../../../.." && pwd)"

# shellcheck disable=SC1091
source "$REPO_ROOT/lib/common.sh"
# shellcheck disable=SC1091
source "$REPO_ROOT/lib/platform.sh"
# shellcheck disable=SC1091
source "$SCRIPT_DIR/metadata.env"

require_os_family "$SCENARIO_FAMILY"

CRON_FILE="/etc/cron.d/lab-cron-report"

info "verifying broken state: $SCENARIO_ID"

[[ -f "$CRON_FILE" ]] || die "cron file is missing"
grep -Eq '^\*/5[[:space:]]+\*[[:space:]]+\*[[:space:]]+\*[[:space:]]+\*[[:space:]]+labmissing[[:space:]]+/usr/local/bin/lab-cron-report\.sh$' \
  "$CRON_FILE" || die "cron job is not using the missing labmissing user"

info "broken state confirmed"
